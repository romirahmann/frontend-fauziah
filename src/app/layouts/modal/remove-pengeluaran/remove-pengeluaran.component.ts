...
@Component({
  selector: 'app-remove-pengeluaran',
  templateUrl: './remove-pengeluaran.component.html',
  styleUrls: ['./remove-pengeluaran.component.scss'],
})
export class RemovePengeluaranComponent {
  ...
  executeTestingFunction() {
    const functionToExecute: keyof RemovePengeluaranComponent | undefined =
  ...
  removePengeluaran() {
    this.apiService.getStok().subscribe((res: any) => {
      ...
      this.apiService.updateStok(1, newStok).subscribe((res: any) => {
        this.apiService
          .deleteQuantity(data.transaction_id)
          .subscribe((res: any) => {
            this.apiService
